...
def train(args):
    ...
    train_dataset = DIV2KDataset(
        root_dir=args.data_root, phase="train", crop_size=args.crop_size
    )
    ...
    valid_dataset = DIV2KDataset(
        root_dir=args.data_root, phase="valid", crop_size=args.crop_size
    )
    ...
    if args.target_final_lr:
        ...
        print(
            f"Calculated Gamma for target final LR {args.target_final_lr}: {calculated_gamma:.6f}"
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if global_step % 100 == 0:
                print(
                    f"Step {global_step}: Loss = {loss.item():.6f}, LR = {scheduler.get_last_lr()[0]:.8f}"
                )

            # Save Checkpoint every 1000 steps
            if global_step > 0 and global_step % 1000 == 0:
                step_save_path = os.path.join(
                    args.save_dir, f"bake_step_{global_step}.pth"
                )
                torch.save(
                    {
                        "epoch": epoch,
        ...
        with torch.no_grad():
            for batch in tqdm(
                valid_loader, desc=f"Epoch {epoch+1}/{args.epochs} [Valid]"
            ):
                x_hr = batch["hr"].to(device)
                x_lr = batch["lr"].to(device)
        ...
        print(
            f"Epoch {epoch+1} Train Loss: {avg_train_loss:.6f}, Valid Loss (EMA): {avg_valid_loss:.6f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_root", type=str, default="./data", help="Path to DIV2K root directory"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-6,
        help="Target final learning rate (overrides gamma)",
    )
    ...
    parser.add_argument(
        "--save_interval", type=int, default=10, help="Save interval in epochs"
    )
    parser.add_argument(
